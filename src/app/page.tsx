      ...
      <div className="fixed inset-0 bg-[#06040a] flex flex-col items-center justify-center font-['Press_Start_2P',monospace] text-white select-none p-4 z-50">
        ...
        <div className="w-full max-w-sm flex flex-col items-center">
          <h1 className="text-[25px] font-black tracking-tighter text-center uppercase mb-1 [text-shadow:4px_4px_0px_#000] flex justify-center gap-[2px] w-full">
            <span className="text-[#3b82f6]">A</span><span className="text-[#4f46e5]">M</span><span className="text-[#5a56e9]">B</span><span className="text-[#6366f1]">I</span><span className="text-[#7c3aed]">E</span><span className="text-[#8b5cf6]">N</span><span className="text-[#9d4edd]">T</span>
            <span className="text-[#a855f7] ml-1.5">D</span><span className="text-[#b566ff]">E</span><span className="text-[#c084fc]">C</span><span className="text-[#d8b4fe]">K</span>
          </h1>
          ...
          <div className="w-full h-8 bg-[#110d20] border-[4px] border-black p-1 relative shadow-[5px_5px_0px_#000]">
            <div className="h-full bg-gradient-to-r from-[#3b82f6] via-[#6366f1] to-[#c084fc] border-r-4 border-black transition-all duration-150 ease-out" style={{ width: `${loadingPercent}%` }} />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <main className="relative w-screen h-screen flex items-center justify-center bg-transparent p-4 font-['Press_Start_2P',monospace]">
      ...
      {minutes === 0 && seconds === 0 && isActive && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-black/90">
          <div className="bg-[#1b1530] border-[6px] border-black p-6 text-center shadow-[8px_8px_0px_#000] max-w-xs w-full">
            <h2 className="text-[10px] text-white mb-6 leading-relaxed">{mode === "work" ? "SESSION DONE!" : "BREAK OVER!"}</h2>
            <button onClick={() => dispatch(resetTimer())} className="w-full border-[4px] border-black bg-[#10b981] text-black py-3 text-[9px] font-bold shadow-[4px_4px_0px_#000] active:translate-x-[4px] active:translate-y-[4px] active:shadow-none transition-all cursor-pointer">
              [ OK ]
            </button>
          </div>
        </div>
      ...
      <div className="w-full max-w-md p-6 bg-[#251c42] border-[6px] border-black shadow-[12px_12px_0px_#000] relative z-10">
        <Screen minutes={minutes} seconds={seconds} mode={mode} isActive={isActive} masterVolume={volumes.master} currentVideoId={YOUTUBE_STREAMS[streamIndex]} initialMinutes={initialMinutes} />

        {mode === "work" ? (
          <div className="grid grid-cols-2 gap-3 text-[8px] mt-4">
            <button onClick={() => dispatch(changePreset("work25"))} className={`border-[4px] border-black py-2.5 font-bold uppercase ${selectedPreset === "work25" ? "bg-[#ff9e3b] text-black shadow-[1px_1px_0px_#000] translate-x-[2px] translate-y-[2px]" : "bg-[#1f2937] text-slate-400 shadow-[3px_3px_0px_#000]"}`}>25M CLASSIC</button>
            <button onClick={() => dispatch(changePreset("work50"))} className={`border-[4px] border-black py-2.5 font-bold uppercase ${selectedPreset === "work50" ? "bg-[#ff9e3b] text-black shadow-[1px_1px_0px_#000] translate-x-[2px] translate-y-[2px]" : "bg-[#1f2937] text-slate-400 shadow-[3px_3px_0px_#000]"}`}>50M DEEP</button>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3 text-[8px] mt-4">
             <button onClick={() => dispatch(changePreset("break5"))} className={`border-[4px] border-black py-2.5 font-bold uppercase ${selectedPreset === "break5" ? "bg-[#10b981] text-black shadow-[1px_1px_0px_#000] translate-x-[2px] translate-y-[2px]" : "bg-[#1f2937] text-slate-400 shadow-[3px_3px_0px_#000]"}`}>5M SHORT</button>
             <button onClick={() => dispatch(changePreset("break15"))} className={`border-[4px] border-black py-2.5 font-bold uppercase ${selectedPreset === "break15" ? "bg-[#10b981] text-black shadow-[1px_1px_0px_#000] translate-x-[2px] translate-y-[2px]" : "bg-[#1f2937] text-slate-400 shadow-[3px_3px_0px_#000]"}`}>15M LONG</button>
          </div>
        )}

        <div className="mt-5">
          {!isMixerOpen ? (
            <Controls onOpenMixer={() => setIsMixerOpen(true)} isActive={isActive} onToggleActive={() => dispatch(toggleTimer())} onReset={() => dispatch(resetTimer())} onNextTrack={handleNextStream} onPrevTrack={handlePrevStream} onShuffleTrack={handleShuffleStream} />
          ) : (
            <div className="border-[4px] border-black bg-[#1b1530] p-4 shadow-[4px_4px_0px_#000] mb-12">
